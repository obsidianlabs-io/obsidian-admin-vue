...
async function loginToPairedBackend(page: Page) {
  ...
  const confirmButton = page.getByRole('button', { name: /Confirm|确定/ }).first();
...
test('frontend pairs with seeded laravel backend for login, tenant switching, and user access', async ({ page }) => {
