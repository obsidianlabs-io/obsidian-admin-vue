...
test('pages preview can open the language drawer in demo runtime', async ({ page }) => {
  ...
  await expect(
    modal.getByPlaceholder(/Ex: route.user \/ common.search|例如：route.user \/ common.search/)
  ).toBeVisible();
...
test('pages preview can save a language record in demo runtime', async ({ page }) => {
  ...
  await modal
    .getByPlaceholder(/Ex: route.user \/ common.search|例如：route.user \/ common.search/)
    .fill(translationKey);
