...
export const useRouteStore = defineStore(SetupStoreId.Route, () => {
   ...
   * "@elegant-router/vue"
   */
  const authRouteMode = ref(import.meta.env.VITE_AUTH_ROUTE_MODE || 'static');
  ...
  function getGlobalMenus(routes: ElegantConstRoute[]) {
    if (authStore.userInfo.menus.length > 0) {
      menus.value = getGlobalMenusByBackend(authStore.userInfo.menus);
      return;
    }
  ...
  async function initDynamicAuthRoute() {
    ...
    if (!error) {
      ...
      const filteredAuthRoutes = filterAuthRoutesByRoles(routes, {
        roles: authStore.userInfo.roles,
        permissionCodes: authStore.userInfo.buttons,
        currentTenantId: authStore.userInfo.currentTenantId,
        routeRules: authStore.userInfo.routeRules
  ...
  function getMenuRouteKey(selectedKey: string) {
    return getRouteKeyByMenuKey(selectedKey, menus.value);
  }
