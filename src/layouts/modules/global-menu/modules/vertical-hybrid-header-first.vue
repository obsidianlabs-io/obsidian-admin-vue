...
function handleSelectMenu(key: string) {
  ...
  const secondFirstMenuKey = secondLevelMenus.value[0]?.key || '';
  if (!secondFirstMenuKey) {
    return;
  }
...
function handleSelectChildLevelMenu(menuKey: string) {
        ...
        >
          ...
          <SimpleScrollbar>
            <NMenu
              ...
              :inverted="inverted"
              :indent="18"
              @update:value="handleSelectChildLevelMenu"
            />
          </SimpleScrollbar>
        </DarkModeContainer>
      </div>
